...
// 2021/01/19 - Chris
// 2021/02/12 - Cornel
// 2021/10/17 - Sophia Huynh finished to-do sections
...
int main(int argc, char** argv)
{
	...
	auto lambda = [usdToCadRate, gbpToCadRate](Book& B) {
	...
	if (argc > 2) {
		...
		if (!file) {
			std::cerr << "file cannot be opened" << argv[2] << std::endl;
			exit(AppErrors::CannotOpenFile);
		}
	...
	catch (std::out_of_range& rangeEnd) {
		std::cout << "** EXCEPTION: " << rangeEnd.what() << std::endl;
	}
